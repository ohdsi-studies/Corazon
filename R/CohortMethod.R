...
#' @param minCohortCount       Any comparison with a cohort with less than minCohortCount will be dropped
#'
#' @export
runCohortMethod <- function(connectionDetails,
  ...
  sql <- SqlRender::render("SELECT cohort_definition_id, count(*) as count_exposures, count(distinct subject_id) as count_persons
  ...
  cohort_counts <- DatabaseConnector::querySql(conn, sql)
  
  names(cohort_counts) <- SqlRender::snakeCaseToCamelCase(names(cohort_counts))
  
  DatabaseConnector::disconnect(conn)
  
  tcosList_tmp <- lapply(tcosList, function(tco){
    
    if(length(cohort_counts$countPersons[cohort_counts$cohortDefinitionId==tco$targetId]) == 0){count_t <- 0}else{count_t <- cohort_counts$countPersons[cohort_counts$cohortDefinitionId==tco$targetId]}
    
    if(length(cohort_counts$countPersons[cohort_counts$cohortDefinitionId==tco$comparatorId]) == 0){count_c <- 0}else{count_c <- cohort_counts$countPersons[cohort_counts$cohortDefinitionId==tco$comparatorId]}
    
    if(count_t >= minCohortCount & count_c >= minCohortCount){
      return(tco)
      }
    ParallelLogger::logWarn(paste0("Dropping T= ", tco$targetId, " / C= ", tco$comparatorId, " due to low or missing counts"))
    ParallelLogger::logWarn(paste0("n(Target)= ",count_t, " / n(Comparator)= " ,count_c ))
    
    return(NULL)
  })
  
  tcosList <- plyr::compact(tcosList_tmp)
...
createTcos <- function(outputFolder) {
  pathToCsv <- system.file("settings", "TcosOfInterest.csv", package = "corazon")
...
getAllControls <- function(outputFolder) {
  ...
  if (file.exists(allControlsFile)) {
    ...
    pathToCsv <- system.file("settings", "NegativeControls.csv", package = "corazon")
